...
//  time_last_good_ros_command_sec = ros::Time::now().toSec();
  ROS_INFO("lin, ang, running_time= %1.2f %1.2f %1.2f" , linear_premap, angular_premap, running_time_desired);
//  return;
...
//  initialize_motors();
  ...
  while(ros::ok()){
	...
	rc_usleep(int(running_time_desired)*1000);
